...
const Conversation = () => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      const userResponse = await axios.get(`http://localhost:8000/api/profiles/${userId}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
              ...
              >
                {messages.length === 0 ? (
                  <div className="text-center py-5">
                    ...
                    <p className="mt-3 mb-0">No messages yet. Start the conversation!</p>
                  </div>
                ) : (
                  <>
                    ...
                    {messages.map((message, index) => {
                      ...
                      return (
                        <React.Fragment key={message.id}>
                          ...
                          <div
                            className={`message-bubble mb-3 ${isReceived ? 'message-sent' : 'message-received'}`}
                          >
                            <div
                              className={`card ${isReceived ? 'bg-primary text-white' : 'bg-light'}`}
                            >
                            ...
                            <small
                              className={`d-block mt-1 ${isReceived ? 'text-end' : ''} text-muted`}
                            >
